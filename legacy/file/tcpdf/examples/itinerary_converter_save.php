...
<table   border="1" cellpadding="1" cellspacing="0">
 ...
 $query_select = "SELECT * FROM  file_entry WHERE   file_count_no ='$file_count_no'  ORDER BY service_date ASC";
		$Num=1;
...
elseif ($dateCon >= $date5from && $dateCon <= $date5to){ $arabic_timeloop=$ee_fills;   }
			...
			$now=$pickup_timeloopSQL;  $dateCon= strtotime($now); //= DateTime::createFromFormat('H:i:s', $now);
...
// <----------------------- getting Service Name Arabic ---------->
   			$Serviceloop_sqls="SELECT * FROM   service WHERE  service_name_english  = '$serviceloopEng'  ORDER BY service_id DESC LIMIT 1";
...
$loop_body_part ='  <tr>
    <td width="116"  height="38" align="center" ><table><tr><td align="right">'. $arabic_timeloop. ' </td><td align="center"> ' .date("g:i", strtotime($flight_timeloop)) .'</td></tr></table></td>
    <td width="128" align="center">'.$LocationFrom_name_arabicLocationloop.'</td>
    <td width="315" align="center"><table><tr><td colspan="2">'.$service_name_arabicServiceloop.'</td></tr><tr><td>
	
  <table><tr><td width="30">'.$Pick_Up_Point_fills.'</td><td width="5">:</td><td  width="80"> '.$from_loop.'</td></tr></table>
</td><td><table><tr width="30"><td>'.$Drop_Off_Point_fills .'</td><td width="5">:</td><td width="80">'.$to_loop.'</td></tr></table></td></tr></table></td>
