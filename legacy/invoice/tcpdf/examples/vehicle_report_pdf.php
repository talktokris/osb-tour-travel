...
if (isset($_GET['vehicle'])) {$tour_came = $_GET['vehicle'];
...
$entry_state_result_conut = mysql_num_rows(mysql_query("SELECT * FROM   file_entry  WHERE vehicle_type = '$search_word'  AND service_type='$service_type' ORDER BY file_count_no DESC"));

if ($entry_state_result_conut>=1) { 
    $resultsHeader = mysql_query("SELECT  *  FROM file_entry WHERE  vehicle_type = '$search_word'  AND service_type='$service_type' ORDER BY file_id DESC LIMIT 1");
...
<table width="960" border="1" cellpadding="5" >
  ...
  <tr>
   ...
   $entry_state_result =  mysql_query("SELECT  *  FROM file_entry WHERE  vehicle_type  = '$search_word' AND service_type='$service_type' AND service_date BETWEEN '$from_date' AND '$to_date' ORDER BY service_date DESC");
