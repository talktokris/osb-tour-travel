...
if (isset($_GET['agent'])) {$agent_came = $_GET['agent'];
$agent_came_part=explode('|', $agent_came);


$search_word=$agent_came_part[0]; // $b=$search_word_part[1];
$from_date=$agent_came_part[1];
$to_date=$agent_came_part[2];
...
$header_table='<table>
<tr><td colspan="2" align="center"><h2>Agent Report</h2> </td></tr>
...
$rowHeader= '<table  border="1"  cellpadding="5" ><tr bgcolor="#cccccc"><td>S.N</td><td>File No.</td><td>Service Name</td><td>Guest Name</td><td>Agent Name</td><td>Vehicle Type</td><td>Pick Up</td><td>Drop off</td><td>Service Date</td><td>Create Date</td><td>No. of Pax</td><td>Driver Name</td><td>Vehicle No.</td></tr>';

$entry_state_result =  mysql_query("SELECT  *  FROM file_entry WHERE  agent_name  = '$search_word' AND date BETWEEN '$from_date' AND '$to_date' ORDER BY date DESC");
while($rows=mysql_fetch_array($entry_state_result)){

$rowOnePart= "<tr height ='30'>
<td>".$Num."</td><td>".$rows['file_no']."</td><td>".$rows['service']."</td><td>".$rows['first_name']."</td><td>".$rows['agent_name']."</td><td>".$rows['vehicle_type']."</td><td>".$rows['from_location']."</td><td>".$rows['to_location']."</td><td>".$rows['service_date']."</td><td>".$rows['date']."</td><td>".$rows['no_of_pax']."</td><td>".$rows['driver_name']."</td><td>".$rows['vehicle_no']."</td>
</tr>";
