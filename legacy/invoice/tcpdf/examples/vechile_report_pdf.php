...
if (isset($_GET['vechile'])) {$tour_came = $_GET['vechile'];
...
$entry_state_result_conut = mysql_num_rows(mysql_query("SELECT * FROM   file_entry  WHERE vehicle_no = '$search_word'  AND service_type='$service_types' ORDER BY file_count_no DESC"));

if ($entry_state_result_conut>=1) { 
    $resultsHeader = mysql_query("SELECT  *  FROM file_entry WHERE  vehicle_no = '$search_word'  AND service_type='$service_types' ORDER BY service_date DESC LIMIT 1");
...
<table width="960" border="1" cellpadding="5" >
  ...
  <tr>
    <td colspan="15" align="left"><h2>'.$service_types.'</h2> </td>
  </tr>
  <tr>
   ...
   $entry_state_result =  mysql_query("SELECT  *  FROM file_entry WHERE  vehicle_no  = '$search_word' AND service_type='$service_types' AND service_date BETWEEN '$from_date' AND '$to_date' ORDER BY service_date DESC");
