...
<?php $rowClass='grid grid-cols-1 md:grid-cols-[220px_1fr] items-center gap-2 px-3 py-1.5'; $labelClass='font-semibold text-sm text-base-content/80'; foreach($m['fields'] as $fld): if(!empty($fld['create_only'])) continue; $k=$fld['key']; $val = (string)($row[$k]??''); if (!empty($fld['arabic']) && function_exists('normalize_arabic_text')) { $val = normalize_arabic_text($val); } ?><div class="<?= $rowClass ?>"><div class="<?= $labelClass ?>"><?= h($fld['label']) ?> :</div><div class="text-sm<?= !empty($fld['arabic']) ? ' text-right' : '' ?>"<?= !empty($fld['arabic']) ? ' dir="rtl" lang="ar"' : '' ?>><?= h($val) ?></div></div><?php endforeach; ?>
