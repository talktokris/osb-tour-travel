...
<dialog id="delete-vehicle-modal" class="agent-delete-dialog"><div class="agent-delete-dialog__surface"><h3 class="font-bold text-lg mb-1">Delete vehicle?</h3><p class="agent-delete-dialog__message" id="delete-vehicle-msg">Are you sure?</p><div class="agent-delete-dialog__actions"><button type="button" class="btn btn-outline" id="delete-vehicle-no">No</button><button type="button" class="btn btn-error" id="delete-vehicle-yes">Yes</button></div></div></dialog>
<style>#delete-vehicle-modal.agent-delete-dialog{margin:0;max-width:none;max-height:none;width:100%;height:100%;padding:1.25rem;border:none;background:transparent;display:none;align-items:center;justify-content:center;box-sizing:border-box}#delete-vehicle-modal.agent-delete-dialog[open]{display:flex}#delete-vehicle-modal.agent-delete-dialog::backdrop{background:rgba(15,23,42,.55);backdrop-filter:blur(4px)}#delete-vehicle-modal .agent-delete-dialog__surface{width:100%;max-width:26rem;background:var(--color-base-100,#fff);border-radius:1rem;padding:1.5rem 1.5rem 1.25rem}#delete-vehicle-modal .agent-delete-dialog__message{margin:0 0 1.35rem;font-size:.875rem;line-height:1.55}#delete-vehicle-modal .agent-delete-dialog__actions{display:flex;gap:.5rem;justify-content:flex-end}</style>
<script>(function(){var m=document.getElementById('delete-vehicle-modal'),f=document.getElementById('delete-vehicle-form'),i=document.getElementById('delete-vehicle-id'),msg=document.getElementById('delete-vehicle-msg'),n=document.getElementById('delete-vehicle-no'),y=document.getElementById('delete-vehicle-yes');if(!m||!f||!i||!msg||!n||!y)return;document.querySelectorAll('.js-delete-vehicle').forEach(function(b){b.addEventListener('click',function(){i.value=this.getAttribute('data-id')||'';var name=this.getAttribute('data-name')||'';msg.textContent=name?'Are you sure you want to delete vehicle "'+name+'"?':'Are you sure?';m.showModal();});});n.addEventListener('click',function(){m.close();});y.addEventListener('click',function(){m.close();f.submit();});})();</script>
