...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!setup_agents_csrf_validate((string) ($_POST['_token'] ?? ''))) {
        ...
        $result = setup_agents_create($mysqli, $form);
        if (!empty($result['ok'])) {
            $newId = (int) ($result['id'] ?? 0);
            setup_agents_flash_set('success', 'Agent created successfully.');
            header('Location: index.php?page=setup_agent_view&id=' . $newId);
            exit;
        }
...
<div class="flex gap-6 w-full">
    ...
    <main class="flex-1 px-4">
        <div class="space-y-4">
            ...
            $breadcrumbCurrent = 'Create Agent';
            ...
            <div class="card bg-base-100 shadow-xl border border-base-300">
                <div class="card-body space-y-4">
                    ...
                    <form method="post" action="index.php?page=setup_agent_create" class="space-y-3">
                        <input type="hidden" name="_token" value="<?= h($csrf) ?>">
                        <div class="max-w-4xl mx-auto border border-base-300 rounded-box overflow-hidden">
                            <div class="px-4 py-2.5 bg-linear-to-r from-sky-700 to-cyan-600 text-white font-bold text-base">Create Agent</div>
                            <div class="divide-y divide-base-300">
                                ...
                                <div class="<?= $rowClass ?>"><label class="<?= $labelClass ?>">Country :</label>
                                    <select name="agent_country" id="create-agent-country" class="<?= $selectClass ?>" required>
                                ...
                                <div class="<?= $rowClass ?>"><label class="<?= $labelClass ?>">City :</label>
                                    <select name="agent_city" id="create-agent-city" class="<?= $selectClass ?>" required>
...
<script>
(function () {
    var co = document.getElementById('create-agent-country');
    var ci = document.getElementById('create-agent-city');
    if (!co || !ci) return;
    function sync() {
        var c = co.value || '';
