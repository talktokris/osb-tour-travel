...
<div class="invoice-form-fieldstack">
  ...
  <label class="form-control w-full max-w-md"><span class="label-text text-xs font-semibold">Supplier Name</span>
    <select name="search_word" class="select select-bordered select-sm w-full bg-white"><option value="">Select</option><?php $sel=(string)($fv['search_word']??''); foreach($suppliers as $a){$s=$sel===$a?' selected':''; echo '<option value="'.h($a).'"'.$s.'>'.h($a).'</option>'; } ?></select>
  </label>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
