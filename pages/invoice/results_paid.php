...
<div class="overflow-x-auto">
  <table class="table table-xs table-bordered w-full min-w-[900px] text-xs whitespace-nowrap">
    ...
    <?php foreach($rows as $r): $p=invoice_module_payment_rows($mysqli, (string)($r['invoices_id'] ?? '')); ?>
      <tr>
        <td><?= h($r['invoices_id']) ?></td><td><?php if (($r['invoice_type'] ?? '') === 'Supplier Invoice'): ?><a class="link" target="_blank" href="index.php?page=invoice_pdf_supplier_converter&file_count_no=<?= h(rawurlencode((string) ($r['file_count_no'] . '|' . $r['agent_supplier_name']))) ?>"><?= h($r['agent_supplier_name']) ?></a><?php else: ?><a class="link" target="_blank" href="index.php?page=invoice_pdf_converter&file_count_no=<?= h(rawurlencode($r['file_count_no'])) ?>"><?= h($r['agent_supplier_name']) ?></a><?php endif; ?></td><td><?= h($r['invoice_type']) ?></td><td><?= h($r['total_price']) ?></td><td><?= h($r['invoice_create_date']) ?></td><td><?= h($r['file_no']) ?></td><td><?= h($r['ref_no']) ?></td><td><?= h($r['paid_status']) ?></td>
        <td>
          <?php if ($p===[]): ?>-<?php else: ?>
